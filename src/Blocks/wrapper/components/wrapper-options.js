...
export const WrapperOptions = ({ attributes, setAttributes }) => {
	...
	return (
		<>
			{!wrapperDisable &&
				<PanelBody title={<span>{__('Layout', 'evo')}</span>} initialOpen={false} icon={icons.wrapper} className='es-panel-title'>
					...
					{(wrapperUse && wrapperUseSimpleShowControl) &&
						<IconToggle
							...
							label={__('Simple wrapper', 'evo')}
							checked={wrapperUseSimple}
							onChange={(value) => setAttributes({ wrapperUseSimple: value })}
							help={__('Provides only basic spacing and background options.', 'evo')}
						/>
					}
					...
					{wrapperUse &&
						<>
							...
							{showWrapperSpacingTop &&
								<Responsive label={<IconLabel icon={icons.spacingTop} label={__('Top spacing', 'evo')} />}>
							...
							{showWrapperSpacingBottom &&
								<Responsive label={<IconLabel icon={icons.spacingBottom} label={__('Bottom spacing', 'evo')} />}>
									{options.breakpoints.map((item, index) => {
										const point = ucfirst(item);
							...
							{showWrapperSpacingTopIn &&
								<Responsive label={<IconLabel icon={icons.spacingTopIn} label={__('Top inner spacing', 'evo')} />}>
									{options.breakpoints.map((item, index) => {
										const point = ucfirst(item);
							...
							{showWrapperSpacingBottomIn &&
								<Responsive label={<IconLabel icon={icons.spacingBottomIn} label={__('Bottom inner spacing', 'evo')} />}>
									{options.breakpoints.map((item, index) => {
										const point = ucfirst(item);
							...
							{showWrapperDividerTop &&
								<Responsive label={<IconLabel icon={icons.dividerTop} label={__('Top divider', 'evo')} />}>
									{options.breakpoints.map((item, index) => {
										const point = ucfirst(item);
							...
							{showWrapperDividerBottom &&
								<Responsive label={<IconLabel icon={icons.dividerBottom} label={__('Bottom divider', 'evo')} />}>
									{options.breakpoints.map((item, index) => {
										const point = ucfirst(item);
							...
							{showWrapperHide &&
								<Responsive label={<IconLabel icon={icons.hide} label={__('Hide', 'evo')} />}>
									{options.breakpoints.map((item, index) => {
										const point = ucfirst(item);
							...
							{showWrapperBackgroundColor &&
								<ColorPaletteCustom
									label={<IconLabel icon={icons.backgroundTypeAlt} label={__('Background color', 'evo')} />}
							...
							{!wrapperUseSimple &&
								<>
									{showWrapperWidth &&
										<Responsive label={<IconLabel icon={icons.width} label={__('Width', 'evo')} />}>
											{options.breakpoints.map((item, index) => {
									...
									{showWrapperOffset &&
										<Responsive label={<IconLabel icon={icons.offset} label={__('Offset', 'evo')} />}>
											{options.breakpoints.map((item, index) => {
									...
									{showWrapperContainerWidth &&
										<Responsive label={<IconLabel icon={icons.containerWidth} label={__('Container width', 'evo')} />}>
											{options.breakpoints.map((item, index) => {
												const point = ucfirst(item);
									...
									{showWrapperGutter &&
										<Responsive label={<IconLabel icon={icons.gutter} label={__('Gutter', 'evo')} />}>
											{options.breakpoints.map((item, index) => {
							...
							{showWrapperAnchorId &&
								<TextControl
									label={<IconLabel icon={icons.anchor} label={__('Block anchor ID', 'evo')} />}
							...
							{showWrapperId &&
								<TextControl
									label={<IconLabel icon={icons.id} label={__('Block unique identifier', 'evo')} />}
									value={wrapperId}
