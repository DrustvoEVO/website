...
export const ColumnsOptions = ({ attributes, setAttributes }) => {
	...
	return (
		<PanelBody title={__('Columns', 'evo')}>
			<HelpModal
				type="columns"
				buttonLabel={__('How to use Columns?', 'evo')}
				modalLabel={__('Columns', 'evo')}
			/>

			<br /><br />

			<Responsive label={<IconLabel icon={icons.gutter} label={__('Column spacing', 'evo')} />}>
			...
			<Responsive label={<IconLabel icon={icons.verticalSpacing} label={__('Row spacing', 'evo')} />}>
				{breakpoints.map((keyName) => {
					const point = ucfirst(keyName);
