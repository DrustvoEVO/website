...
export const ColumnOptions = ({ attributes, setAttributes }) => {
	...
	return (
		<PanelBody title={__('Column', 'evo')}>

			<HelpModal
				type='column'
				buttonLabel={__('How to use a Column?', 'evo')}
				modalLabel={__('Column', 'evo')}
			/>
			...
			<Responsive label={<IconLabel icon={icons.order} label={__('Order', 'evo')} />}>
				{breakpoints.map((keyName, index) => {
					...
					return (
						<Fragment key={keyName}>
							...
							/>
							{index === 0 &&
								<p className='es-decorative-text'>{__('Needs to be set on all columns!', 'evo')}</p>
							}
						</Fragment>
					);
				})}
			...
			<Responsive label={<IconLabel icon={icons.verticalAlign} label={__('Vertical align', 'evo')} />}>
				{breakpoints.map((keyName, index) => {
					const point = ucfirst(keyName);
			...
			<Responsive label={<IconLabel icon={icons.horizontalAlign} label={__('Horizontal align', 'evo')} />}>
				{breakpoints.map((keyName, index) => {
					const point = ucfirst(keyName);
