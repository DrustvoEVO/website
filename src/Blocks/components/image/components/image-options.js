	...
	return (
		<>
			...
			{imageUse &&
				<>
					{showImageUrl &&
						<Responsive label={<IconLabel icon={icons.image} label={__('Image source', 'evo')} />}>
							{Object.keys(checkAttrResponsive('imageUrl', attributes, manifest)).map(function (keyName) {
								...
								if (point === 'Large') {
									point = '';
									pointLabel = __('Default (all screens)', 'evo');
								}
					...
					{showImageAlt &&
						<TextareaControl
							label={<IconLabel icon={icons.altText} label={__('Alt text', 'evo')} />}
							value={imageAlt}
					...
					{showImageFull &&
						<IconToggle
							icon={icons.size}
							label={__('Fill container', 'evo')}
							help={__('Span the full width of the container and ignore its maximum width', 'evo')}
							checked={imageFull}
